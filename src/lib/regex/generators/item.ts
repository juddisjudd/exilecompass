...
// Selected mods are keyed `${baseType}::${modCategory}::${description}` and
...
export function itemModKey(baseType: string, category: string, mod: ItemModifier): string {
  return `${baseType}::${category}::${mod.description}`;
}
...
function modsForBase(data: ItemRegex[], baseType: string): Map<string, ItemModifier> {
  ...
  for (const cat of entry?.itemRegexForCategory ?? []) {
    for (const mod of cat.modifiers) map.set(itemModKey(baseType, cat.modCategory, mod), mod);
  }
  return map;
}
...
function modRegex(mod: ItemModifier, values: Record<number, string>): string {
  ...
  const onIndex = mod.regexPosition.on[0];
  const regex =
    onIndex !== undefined && has(onIndex) ? mod.regex.replace('\\d+', `${bounded(onIndex)}.*`) : mod.regex;
  const before = mod.regexPosition.before.filter(has).map(bounded).join('.*');
  const after = mod.regexPosition.after.filter(has).map(bounded).join('.*');
