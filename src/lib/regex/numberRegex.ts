// Generates a "minimum value" regex from a number, with optional rounding to
// the nearest 10 to keep the assembled query short. Ported verbatim from
// poe.re (shared/core/regex/GenerateNumberRegex.ts).
...
// the "(". Falls back to an open-ended ">= min" match when the bounds don't
// fit a 2-digit range. Ported from the same poe.re module.
...
// Shortest regex matching an inclusive [min, max] integer range. Only 1- and
// 2-digit numbers (0-99) are handled; anything else returns "".
export function generateNumberRangeRegex(min: string, max: string, round10: boolean): string {
  ...
  if (minDigits === null || maxDigits === null) return "";
  if (minDigits.length > 2 || maxDigits.length > 2) return "";
  ...
  if (isNaN(lo) || isNaN(hi) || lo < 0 || hi > 99 || hi < lo) return "";

  const parts: string[] = [];
  if (lo <= 9) parts.push(singleDigitPart(lo, Math.min(hi, 9)));
  if (hi >= 10) parts.push(...twoDigitParts(Math.max(lo, 10), hi));
...
function twoDigitParts(lo: number, hi: number): string[] {
  ...
  const parts: string[] = [];
  if (b !== 0) parts.push(b === 9 ? `${a}9` : `${a}[${b}-9]`);
  const fullLo = b === 0 ? a : a + 1;
  const fullHi = d === 9 ? c : c - 1;
  if (fullLo <= fullHi) parts.push(fullLo === fullHi ? `${fullLo}.` : `[${fullLo}-${fullHi}].`);
  if (d !== 9) parts.push(d === 0 ? `${c}0` : `${c}[0-${d}]`);
  return parts;
}
